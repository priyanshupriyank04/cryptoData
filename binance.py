...
# Selected exchanges - BINANCE ONLY
SELECTED_EXCHANGES = [
    'binance'
]
...
def ensure_connection_alive(connection, creds, db_name):
    """Ensure connection is alive, reconnect if needed. Returns (connection, was_reconnected)."""
    try:
    ...
    print(f"      [WARN] Connection lost, reconnecting...")
    try:
        new_connection = get_database_connection(creds, db_name)
        if new_connection and new_connection.is_connected():
            ...
            print(f"      [ERROR] Reconnection failed")
            return None, False
    except Exception as e:
...
def insert_comprehensive_data(connection, table_name, ohlcv_data, ticker_data, orderbook_data, 
                             funding_data, open_interest_data, instrument_type, market_info, creds=None, db_name=None):
    ...
    try:
        ...
        # Connection lost, try to reconnect if credentials provided
        if creds and db_name:
            ...
            try:
                ...
                if connection.is_connected():
                    ...
                    print(f"      [ERROR] Reconnection failed")
                    return -2  # Return -2 to indicate connection failure
            except Exception as reconnect_error:
                print(f"      [ERROR] Reconnection error: {reconnect_error}")
                return -2
        else:
            print(f"      [ERROR] Connection lost and no credentials for reconnect")
            return -2
    ...
    try:
        ...
        error_msg = str(e)
        if 'Lost connection' in error_msg or 'MySQL Connection not available' in error_msg:
            print(f"    ✗ Connection lost during insert: {e}")
            # Try to reconnect if credentials provided
            if creds and db_name:
                try:
                    connection = get_database_connection(creds, db_name)
                    if connection.is_connected():
                        print(f"    [OK] Reconnected, will retry insert")
                        return -2  # Return -2 to indicate need for retry
                except:
                    pass
        else:
...
def fetch_all_public_data(exchange, symbol, instrument_type, market_info, connection, table_name, 
    ...
    try:
        ...
        print(f"      [INFO] MySQL connection status: {'Connected' if connection.is_connected() else 'Disconnected'}")
        print(f"      [INFO] Starting timestamp: {datetime.fromtimestamp(since/1000) if since else 'From listing date'}")
        
        # Fetch additional public data ONCE at the start (not per batch for speed)
        # Add timeouts and logging to prevent hanging
        ticker_data = {}
        orderbook_data = {}
        funding_data = {}
        open_interest_data = {}
        
        print(f"      [INFO] Fetching additional data (ticker, orderbook, etc.)...")
        
        # Fetch ticker data once (skip if fails) - with timeout
        if hasattr(exchange, 'fetchTicker') and exchange.has.get('fetchTicker', False):
            try:
                exchange.timeout = 10000  # 10 second timeout
                ticker = exchange.fetch_ticker(symbol)
                if ticker:
                    ticker_data[int(datetime.now().timestamp() * 1000)] = ticker
                    print(f"      [OK] Ticker data fetched")
            except Exception as e:
                print(f"      [SKIP] Ticker data unavailable: {type(e).__name__}")
        
        # Fetch order book once (skip if fails) - with timeout
        if hasattr(exchange, 'fetchOrderBook') and exchange.has.get('fetchOrderBook', False):
            try:
                ...
                if orderbook:
                    ...
                    print(f"      [OK] Orderbook data fetched")
            except Exception as e:
                print(f"      [SKIP] Orderbook data unavailable: {type(e).__name__}")
        
        # Fetch funding rate for futures once (skip if fails) - with timeout
        if instrument_type in ['future', 'futures', 'swap', 'perpetual']:
            if hasattr(exchange, 'fetchFundingRate') and exchange.has.get('fetchFundingRate', False):
                try:
                    ...
                    if funding and isinstance(funding, dict):
                        ...
                        print(f"      [OK] Funding rate data fetched")
                except Exception as e:
                    print(f"      [SKIP] Funding rate unavailable: {type(e).__name__}")
            
            # Fetch open interest once (skip if fails) - with timeout
            if hasattr(exchange, 'fetchOpenInterest') and exchange.has.get('fetchOpenInterest', False):
                try:
                    ...
                    if oi and isinstance(oi, dict):
                        ...
                        print(f"      [OK] Open interest data fetched")
                except Exception as e:
                    print(f"      [SKIP] Open interest unavailable: {type(e).__name__}")
        
        print(f"      [INFO] Additional data fetch complete, starting OHLCV fetch...")
        ...
        while True:
            try:
                ...
                connection, was_reconnected = ensure_connection_alive(connection, creds, db_name)
                if not connection:
                    print(f"      [ERROR] Cannot proceed without database connection")
                    break
                ...
                inserted = insert_comprehensive_data(
                    ...
                    creds=creds, db_name=db_name
                )
                
                # Handle connection failure (-2 means connection lost)
                if inserted == -2:
                    print(f"      [WARN] Connection lost during insert, reconnecting...")
                    try:
                        connection = get_database_connection(creds, db_name)
                        if connection.is_connected():
                            print(f"      [OK] Reconnected, retrying insert...")
                            # Retry the insert
                            inserted = insert_comprehensive_data(
                                connection, table_name, ohlcv_batch, ticker_data, orderbook_data,
                                funding_data, open_interest_data, instrument_type, market_info,
                            ...
                            if inserted == -2:
                                print(f"      [ERROR] Insert failed after reconnect, skipping batch")
                                continue
                        else:
                            print(f"      [ERROR] Reconnection failed, skipping batch")
                            continue
                    except Exception as reconnect_error:
                        print(f"      [ERROR] Reconnection error: {reconnect_error}")
                        continue
        ...
        for market_type, market_list in all_markets:
            ...
            for symbol, market_info in market_list:
                ...
                for timeframe in sorted(timeframes):
                    ...
                    try:
                        # Ensure connection is alive before processing this instrument
                        connection, _ = ensure_connection_alive(connection, creds, db_name)
                        if not connection:
                            print(f"    [ERROR] Cannot proceed without database connection")
                            break
                        ...
                        print(f"    [INFO] Checking table status...")
                        was_cleared = check_and_clear_if_incomplete(connection, table_name)
                        print(f"    [INFO] Table check complete, checking if data is up to date...")
                        ...
                        if not connection.is_connected():
                            print(f"    [ERROR] MySQL connection lost, reconnecting...")
                            try:
                                connection = get_database_connection(creds, db_name)
                                if not connection.is_connected():
                                    print(f"    [ERROR] Failed to reconnect, skipping...")
                                    continue
                            except Exception as e:
                        ...
                        print(f"    [INFO] Starting data fetch for {symbol} ({market_type}) - {timeframe}...")
