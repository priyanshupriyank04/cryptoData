...
CHECKPOINT_FILE = os.path.join(PROJECT_ROOT, 'crypto_data', 'extraction_checkpoint.json')
...
def check_and_clear_if_incomplete(connection, table_name):
    """
    Check if table's last entry is from today.
    ...
    try:
        ...
        # Check if last timestamp is from today
        today = datetime.now().date()
        last_date = last_timestamp.date() if isinstance(last_timestamp, datetime) else last_timestamp
        
        if last_date < today:
            # Last entry is NOT from today - clear table and restart
            print(f"      ⚠ Last entry is from {last_date}, clearing table to restart...")
            cursor.execute(f"TRUNCATE TABLE `{table_name}`")
            connection.commit()
            cursor.close()
            print(f"      ✓ Table cleared, restarting fetch...")
            return True  # Indicates table was cleared, needs restart
        
        # Last entry is from today - data is up to date
        cursor.close()
        return False
...
def fetch_all_public_data(exchange, symbol, instrument_type, market_info, connection, table_name, 
    ...
    try:
        # Check if table has data and if last entry is from today
        # If not from today, clear table and restart
        was_cleared = check_and_clear_if_incomplete(connection, table_name)
        
        # If table was cleared, we'll start from listing date
        # If table has data from today, skip it
        if not was_cleared and table_has_data(connection, table_name):
            return -1, None  # Return -1 to indicate skipped (data is up to date)
